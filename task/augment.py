...
def recon_training(args):
    ...
    for i, batch_iter in enumerate(tqdm(dataloader_dict['train'], bar_format='{l_bar}{bar:30}{r_bar}{bar:-2b}')):
        ...
        latent_out = encoder_out_copy - (0.3 * hidden_states_grad)

        for i in range(5):
            model.zero_grad() 
            
            encoder_out_grad_true = latent_out_copy.clone().detach().requires_grad_(True)
            logit = model.classify(encoder_hidden_states=encoder_out_grad_true, encoder_attention_mask=src_att)

            print(torch.softmax(logit, dim=1))

            criterion = nn.CrossEntropyLoss().to(device)
            cls_loss = criterion(logit, fliped_trg_label.float())
            cls_loss.backward()

            hidden_states_grad = encoder_out_grad_true.grad.data.sign()

            encoder_out_copy = encoder_out.clone().detach()
            latent_out_copy = encoder_out_copy - (0.3 * hidden_states_grad)

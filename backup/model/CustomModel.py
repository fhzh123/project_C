...
class CustomModel(nn.Module):
    def __init__(self, encoder_model_type: str = 'bart', decoder_model_type: str = 'bart',
                 ...
                 src_max_len: int = 150, trg_max_len: int = 150, isPreTrain: bool = True, 
                 dropout: float = 0.3, pca_reduction: bool = False, pca_comp_n: int = 5, pooling_method: str = 'first_token'):
        super().__init__()
        ...
        self.encoder_model_name = return_model_name(self.encoder_model_type)
        self.encoder, self.encoder_model_config = encoder_model_setting(self.encoder_model_name, self.isPreTrain)

        try:
            self.enc_d_hidden = self.encoder_model_config.d_model
        except:
            self.enc_d_hidden = self.encoder_model_config.hidden_size
        ...
        self.decoder_model_name = return_model_name(self.decoder_model_type)
        self.decoder, self.decoder_model_config = decoder_model_setting(self.decoder_model_name, self.isPreTrain)

        try:
            self.dec_d_hidden = self.decoder_model_config.d_model
        except:
            self.dec_d_hidden = self.decoder_model_config.hidden_size
    ...
    def pooling(self, hidden_states, eos_mask=None):
        if self.pooling_method == 'first_token':
            hidden_states = hidden_states[:,0,:] # (batch_size, d_hidden)
        if self.pooling_method == 'last_token':
            hidden_states = hidden_states[eos_mask, :].view(hidden_states.size(0), -1, hidden_states.size(-1))
            hidden_states = hidden_states[:,-1,:] # (batch_size, d_hidden)
        if self.pooling_method == 'max':
            hidden_states = hidden_states.max(dim=1)[0] # (batch_size, d_hidden)
        if self.pooling_method == 'mean':
            hidden_states = hidden_states.mean(dim=1) # (batch_size, d_hidden)

        return hidden_states
    
    def pca_pooling(self, hidden_states):
        U, S, V = torch.pca_lowrank(hidden_states.transpose(1,2), q=self.pca_comp_n)
        return U, S, V

    def pca_inverse(self, U, S, V):
        approx_x = U.matmul(torch.diag_embed(S, 0)).matmul(V.transpose(1,2))
        return approx_x
    ...
    def classify(self, hidden_states, eos_mask=None):
        ...
        hidden_states = self.dropout(F.gelu(self.cls_linear(hidden_states)))
        logits = self.cls_linear2(self.cls_norm(hidden_states))
    ...
    def forward(self, src_input_ids, src_attention_mask, trg_input_ids, decoder_start_token_id, eos_mask=None):
        hidden_states = self.encode(src_input_ids, src_attention_mask)
        if self.encoder_model_type in ['bart', 'T5']:
            hidden_states = self.decode(trg_input_ids, decoder_start_token_id, hidden_states, src_attention_mask)
        
        if self.pooling_method == 'pca':
            hidden_states = self.pca_pooling(hidden_states=hidden_states)
        else:
            hidden_states = self.pooling(hidden_states, eos_mask=eos_mask)

        logits = self.classify(hidden_states=hidden_states)
